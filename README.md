...
- **Backend API** (Node/Express or Python/Flask) — proxies WHOIS, Safe Browsing, and homograph-distance checks, caches results, and hosts the optional enrichment endpoint (keeps API keys off the client)
...
2. Deploy the backend (or run locally) with API keys for Google Safe Browsing and a WHOIS/RDAP provider. An LLM provider key is optional — without it, the extension falls back to template-generated messages.
...
- **Local-first is the goal:** detection, scoring, and message generation all run entirely in the browser. Only reputation lookups and the optional enrichment call touch a backend.
